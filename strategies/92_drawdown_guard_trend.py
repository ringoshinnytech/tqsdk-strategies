...
【策略介绍】
回撤保护趋势策略用于演示如何把“趋势信号有效时持仓，若价格相对近期高低点回撤过深则降低方向暴露”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 INE.sc2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：均线斜率、回撤保护项、20 根 K 线动量。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"trend_slope": 0.9, "drawdown_guard": 0.8, "momentum_20": 0.3}
EPS = 1e-12
