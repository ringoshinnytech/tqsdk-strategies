...
【策略介绍】
RSI 背离反转策略用于演示如何把“价格创新高低而 RSI 动能不确认时，捕捉短周期反转机会”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 DCE.m2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：RSI 超买超卖反转、价格与动量背离、价格 Z-Score 回归。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"rsi_reversal": 1.0, "divergence": 0.9, "mean_reversion": 0.4}
EPS = 1e-12
