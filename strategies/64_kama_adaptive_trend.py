...
【策略介绍】
KAMA 自适应均线趋势策略用于演示如何把“用效率系数调节均线快慢，在噪音较低时跟随趋势、震荡时降低换手”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.rb2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：KAMA 自适应均线方向、价格路径效率、波动惩罚项。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"kama_trend": 1.25, "efficiency": 0.7, "volatility_filter": -0.25}
EPS = 1e-12
