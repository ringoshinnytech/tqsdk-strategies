...
【策略介绍】
ATR 标准化 Z-Score 回归策略用于演示如何把“用 ATR 标准化价格偏离，避免高波动阶段误判普通波动为极端偏离”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.au2506，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：ATR 标准化偏离回归、价格 Z-Score 回归、波动惩罚项。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
STRATEGY_ID = 73
STRATEGY_NAME = "ATR 标准化 Z-Score 回归策略"
SYMBOLS = ["SHFE.au2506"]
MODE = "single"
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"atr_zscore_reversion": 1.3, "mean_reversion": 0.5, "volatility_filter": -0.15}
EPS = 1e-12
