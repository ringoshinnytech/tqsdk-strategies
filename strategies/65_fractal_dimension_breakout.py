...
【策略介绍】
分形维度过滤突破策略用于演示如何把“用分形维度识别趋势化行情，仅在价格突破且路径效率较高时入场”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.cu2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：分形维度趋势过滤、近期通道突破幅度、成交量确认。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"fractal_trend": 1.05, "range_breakout": 0.85, "volume_confirm": 0.35}
EPS = 1e-12
