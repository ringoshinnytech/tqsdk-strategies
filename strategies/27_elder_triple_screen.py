...
【策略介绍】
Elder 三屏交易系统用于演示如何把一个明确的交易想法落到 TqSdk 策略脚本中。核心思路是：日线MACD趋势+小时线Stochastic超买超卖+15分钟均线精确入场
脚本默认关注 SHFE.rb2510，运行时先订阅行情或 K 线数据，再计算对应的指标、价差、排名或过滤条件；当信号满足要求时，策略会调整模拟账户持仓，信号消失或风险条件触发时退出。
