...
【策略介绍】
成交密集区价值回归策略用于演示如何把“用近似成交量分布估计价值区，价格远离高成交区域后做回归”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 DCE.y2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：成交密集区回归、成交量确认、价格 Z-Score 回归。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"value_area_reversion": 1.1, "volume_confirm": 0.4, "mean_reversion": 0.4}
EPS = 1e-12
