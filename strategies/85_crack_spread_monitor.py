...
【策略介绍】
裂解价差监控对冲策略用于演示如何把“观察原油与燃料油价差变化，价差极端偏离时做相对价值对冲”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 INE.sc2505、SHFE.fu2505，使用 配对/价差交易：围绕两个合约的价差或比价做均值回归或顺势扩张。
核心因子包括：价差 Z-Score、价差动量。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
STRATEGY_ID = 85
STRATEGY_NAME = "裂解价差监控对冲策略"
SYMBOLS = ["INE.sc2505", "SHFE.fu2505"]
MODE = "pair"
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"spread_zscore": 1.0, "spread_momentum": 0.3}
EPS = 1e-12
