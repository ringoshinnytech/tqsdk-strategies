...
【策略介绍】
日内 VWAP 标准差带回归策略用于演示如何把“围绕 VWAP 构建动态偏离带，价格远离成交均价后等待回归”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.rb2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：VWAP 偏离回归、RSI 超买超卖反转、成交量确认。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
STRATEGY_ID = 69
STRATEGY_NAME = "日内 VWAP 标准差带回归策略"
SYMBOLS = ["SHFE.rb2505"]
MODE = "single"
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"vwap_reversion": 1.3, "rsi_reversal": 0.5, "volume_confirm": -0.2}
EPS = 1e-12
