...
【策略介绍】
ATR 追踪加仓趋势策略用于演示如何把“趋势确认后用 ATR 管理追踪止损，并按信号强度逐步提高目标手数”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.rb2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：均线斜率、ATR 标准化价格位置、20 根 K 线动量。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 3
FACTOR_WEIGHTS = {"trend_slope": 0.8, "atr_position": 0.5, "momentum_20": 0.5}
EPS = 1e-12
