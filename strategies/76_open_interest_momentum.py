...
【策略介绍】
持仓量动量确认策略用于演示如何把“用价格动量叠加持仓量变化确认新增资金是否支持趋势延续”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 DCE.j2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：持仓量动量、20 根 K 线动量、成交量确认。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"oi_momentum": 1.0, "momentum_20": 0.7, "volume_confirm": 0.4}
EPS = 1e-12
