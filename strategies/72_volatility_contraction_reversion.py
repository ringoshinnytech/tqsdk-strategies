...
【策略介绍】
波动收缩震荡回归策略用于演示如何把“当 ATR 相对历史收缩时，降低追涨杀跌并交易区间内均值回归”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.ni2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：波动收缩状态、价格 Z-Score 回归、区间内反转。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"volatility_decline": 0.8, "mean_reversion": 1.0, "range_reversion": 0.5}
EPS = 1e-12
