...
【策略介绍】
换手放大突破策略用于演示如何把“用成交活跃度突增确认突破有效性，减少低流动性环境中的噪声交易”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 CZCE.MA505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：换手放大突破、近期通道突破幅度、短周期动量。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"turnover_breakout": 1.1, "range_breakout": 0.6, "momentum_5": 0.25}
EPS = 1e-12
