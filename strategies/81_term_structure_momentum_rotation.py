...
【策略介绍】
期限结构动量轮动策略用于演示如何把“用期限结构斜率代理库存预期，再与趋势动量共同决定多空轮动”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.rb2505、INE.sc2505、DCE.y2505、CZCE.CF505、SHFE.au2506，使用 多合约截面轮动：对品种池逐一打分，做多相对强者、做空相对弱者。
核心因子包括：期限结构斜率代理、20 根 K 线动量、风险调整动量。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
STRATEGY_ID = 81
STRATEGY_NAME = "期限结构动量轮动策略"
SYMBOLS = ["SHFE.rb2505", "INE.sc2505", "DCE.y2505", "CZCE.CF505", "SHFE.au2506"]
MODE = "cross_section"
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"term_structure_proxy": 1.0, "momentum_20": 0.6, "risk_adjusted_momentum": 0.25}
EPS = 1e-12
