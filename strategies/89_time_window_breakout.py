...
【策略介绍】
交易时段窗口突破策略用于演示如何把“只在预设活跃时段响应突破信号，降低低流动性时段的假信号”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 CFFEX.IF2506，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：交易时段窗口突破、成交量确认、近期通道突破幅度。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
STRATEGY_ID = 89
STRATEGY_NAME = "交易时段窗口突破策略"
SYMBOLS = ["CFFEX.IF2506"]
MODE = "single"
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"opening_range_breakout": 1.2, "volume_confirm": 0.5, "range_breakout": 0.3}
EPS = 1e-12
