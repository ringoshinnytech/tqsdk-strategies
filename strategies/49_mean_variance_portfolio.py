...
【策略介绍】
均值方差最优组合策略用于演示如何把一个明确的交易想法落到 TqSdk 策略脚本中。核心思路是：Markowitz最大夏普比率组合，波动率目标调仓，多品种联合配置
脚本默认关注 SHFE.rb2501、SHFE.cu2501、DCE.i2501、DCE.m2501、INE.sc2501 等多个品种，运行时先订阅行情或 K 线数据，再计算对应的指标、价差、排名或过滤条件；当信号满足要求时，策略会调整模拟账户持仓，信号消失或风险条件触发时退出。
这类组合配置策略适合多品种联合管理，重点在相关性、波动率和资金分散，不应把它当成单品种短线信号。
