...
【策略介绍】
线性回归通道策略用于演示如何把一个明确的交易想法落到 TqSdk 策略脚本中。核心思路是：用线性回归中轴和上下通道衡量趋势斜率与价格偏离，突破或回归时给出交易信号。
脚本默认关注 KQ.m@SHFE.rb，运行时先订阅行情或 K 线数据，再计算对应的指标、价差、排名或过滤条件；当信号满足要求时，策略会调整模拟账户持仓，信号消失或风险条件触发时退出。
这类回归通道策略既可以观察趋势斜率，也可以观察价格相对通道的偏离，使用时要先确认脚本当前采用的是突破还是回归逻辑。
