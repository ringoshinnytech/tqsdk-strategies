...
【策略介绍】
ADX 趋势强度过滤策略用于演示如何把一个明确的交易想法落到 TqSdk 策略脚本中。核心思路是：ADX>25确认趋势强度，+DI/-DI判断方向，ATR追踪止损动态护盈
脚本默认关注 DCE.i2601，运行时先订阅行情或 K 线数据，再计算对应的指标、价差、排名或过滤条件；当信号满足要求时，策略会调整模拟账户持仓，信号消失或风险条件触发时退出。
