...
【策略介绍】
MFI 资金流反转策略用于演示如何把“用资金流量指标识别量价超买超卖，寻找资金动能衰竭后的反转”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.cu2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：MFI 资金流反转、量价背离、RSI 超买超卖反转。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"mfi_reversal": 1.2, "price_volume_divergence": 0.6, "rsi_reversal": 0.25}
EPS = 1e-12
