...
【策略介绍】
Beta 中性配对交易策略用于演示如何把“用滚动 Beta 调整配对合约敞口，在残差偏离时构建中性对冲”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.cu2505、SHFE.al2505，使用 配对/价差交易：围绕两个合约的价差或比价做均值回归或顺势扩张。
核心因子包括：价差 Z-Score、Beta 调整残差。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
STRATEGY_ID = 87
STRATEGY_NAME = "Beta 中性配对交易策略"
SYMBOLS = ["SHFE.cu2505", "SHFE.al2505"]
MODE = "pair"
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"spread_zscore": 1.0, "beta_residual": 0.6}
EPS = 1e-12
