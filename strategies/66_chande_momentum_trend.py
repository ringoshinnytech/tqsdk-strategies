...
【策略介绍】
Chande 动量振荡趋势策略用于演示如何把“用 CMO 衡量净动量强度，并叠加中期收益率确认趋势延续”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 INE.sc2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：Chande 动量强度、20 根 K 线动量、成交量确认。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"cmo": 1.2, "momentum_20": 0.55, "volume_confirm": 0.25}
EPS = 1e-12
