...
【策略介绍】
波动率目标仓位策略用于演示如何把“用风险预算约束目标手数，高波动时自动降仓、低波动趋势中适度增仓”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.cu2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：风险调整动量、波动率目标仓位、均线斜率。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 3
FACTOR_WEIGHTS = {"risk_adjusted_momentum": 1.0, "vol_target": 0.6, "trend_slope": 0.3}
EPS = 1e-12
