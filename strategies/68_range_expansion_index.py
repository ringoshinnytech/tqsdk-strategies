...
【策略介绍】
区间扩张强度突破策略用于演示如何把“用真实波幅扩张和短期动量判断突破质量，过滤弱波动假突破”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.al2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：真实波幅扩张、短周期动量、ATR 标准化价格位置。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"range_expansion": 1.1, "momentum_5": 0.5, "atr_position": 0.35}
EPS = 1e-12
