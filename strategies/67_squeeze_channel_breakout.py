...
【策略介绍】
窄幅压缩通道突破策略用于演示如何把“先识别布林带宽收缩，再等待价格突破近期通道并配合放量确认”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 DCE.i2505，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：低波动压缩后的突破、近期通道突破幅度、成交量确认。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 1
FACTOR_WEIGHTS = {"squeeze_breakout": 1.3, "range_breakout": 0.7, "volume_confirm": 0.45}
EPS = 1e-12
