...
【策略介绍】
自适应仓位 sizing 策略用于演示如何把“综合趋势效率、波动目标和行情状态动态调整目标持仓规模”这一交易想法落到 TqSdk 策略脚本中。
脚本默认关注 SHFE.au2506，使用 单合约时序信号：根据加权因子得分决定目标多空仓位。
核心因子包括：趋势/震荡状态识别、价格路径效率、波动率目标仓位。核心因子会被标准化并加权成综合得分，得分超过入场阈值时建立目标仓位，得分回落到退出阈值附近时降低或清空仓位。
...
BASE_VOLUME = 1
MAX_VOLUME = 3
FACTOR_WEIGHTS = {"regime_trend": 0.7, "efficiency": 0.6, "vol_target": 0.6}
EPS = 1e-12
